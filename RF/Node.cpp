...
template<typename T>
void getFeatureRange(const shared_ptr<vector<shared_ptr<vector<T> > > > i_dataSet, int featureIndx,
                     T *o_min,T *o_max)
{
    T min=100000;
    T max=-100000;
...
template<typename T>
void Node<T>::binSplitDataSet(const shared_ptr<vector<int> > i_indexList, int featureIdx,  T featureValue,
...
template<typename T>
void Node<T>::GetFeatureRange(int fIndex,T * min,T * max)
{
   	T tempMin=100000;
    T tempMax=-100000;
    for (int i=0;i<sampleIndexList->size();i++)
...
template<typename T>
double Node<T>::impurityLeaf(const shared_ptr<vector<int> > i_sampleIndexList)
{
    //	double sum=0;
    //	double sq_sum=0;
...
template<typename T>
double Node<T>::impurityLeafWithWeight(const shared_ptr<vector<int> > i_sampleIndexList, double * w1, double * w0)
{
   	if(i_sampleIndexList==nullptr)return -1;
    double N0=0;
    double N1=0;
...
template<typename T>
void Node<T>::chooseBestSplit(int * o_bestFeatureIndex, T * o_bestFeatureValue, double * o_decreadedImpurity)
{
    bool singleCls=true;
    ...
    for(int i=0;i<Nfsq;i++)
    {
        ...
        for(int j=0;j<10;j++)
        {
            ...
            shared_ptr<vector<int> > indexList0(new vector<int>);
            shared_ptr<vector<int> > indexList1(new vector<int>);
...
//    shared_ptr<Node<T> > leftchild(new Node<T>);
...
//    shared_ptr<Node<T> >  rightchild(new Node<T>);
...
template<typename T>
void Node<T>::UpdateGiniImportance()
{
    if(featureIndex==-1)
    {
        return;
...
template<typename T>
void Node<T>::UpdateTree(const shared_ptr<vector<int> > i_addSampleList)
{
...
template<typename T>
int Node<T>::UpdateTree(const shared_ptr<vector<int> > i_rmvSampleList, const shared_ptr<vector<int> > i_addSampleList)
{
    ...
    shared_ptr<vector<int> > rmvSampleList0(new vector<int>);
    shared_ptr<vector<int> > rmvSampleList1(new vector<int>);
    shared_ptr<vector<int> > addSampleList0(new vector<int>);
    shared_ptr<vector<int> > addSampleList1(new vector<int>);
...
template<typename T>
void Node<T>::GetSampleList(shared_ptr<vector<int> > o_posSampleList, shared_ptr<vector<int> > o_negSampleList)
{
    if(featureIndex==-1)
    {
        for(int i=0;i<sampleIndexList->size();i++)
...
template<typename T>
double Node<T>::PredictOneSample(const shared_ptr<vector<T> > i_inData)
{
    if(featureIndex==-1)
    {
        return splitValue;
...
template<typename T>
void Node<T>::ConvertTreeToList(int * io_left, int * io_right,
